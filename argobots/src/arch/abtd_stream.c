...
static void *xstream_context_thread_func(void *arg)
{
    ...
    while (1) {
        ...
        do {
            real_pthread_cond_wait(&p_ctx->state_cond, &p_ctx->state_lock);
        ...
        real_pthread_mutex_unlock(&p_ctx->state_lock);
        if (!restart)
            break;
    }
    return NULL;
                                             ...
                                             ABTD_xstream_context *p_ctx)
{
    ...
    ret = real_pthread_mutex_init(&p_ctx->state_lock, NULL);
    if (ret != 0)
        goto FAILED;
    init_stage = 1;

    ret = real_pthread_cond_init(&p_ctx->state_cond, NULL);
    if (ret != 0)
        goto FAILED;
    init_stage = 2;

    ret = real_pthread_create(&p_ctx->native_thread, NULL,
...
FAILED:
    if (init_stage >= 2) {
        ret = real_pthread_cond_destroy(&p_ctx->state_cond);
        ABTI_ASSERT(ret == 0);
    }
    if (init_stage >= 1) {
        ret = real_pthread_mutex_destroy(&p_ctx->state_lock);
        ABTI_ASSERT(ret == 0);
    }
...
void ABTD_xstream_context_free(ABTD_xstream_context *p_ctx)
{
    ...
    if (p_ctx->state == ABTD_XSTREAM_CONTEXT_STATE_UNINIT) {
        ...
        int ret;
        ret = real_pthread_join(p_ctx->native_thread, NULL);
        ABTI_ASSERT(ret == 0);
        ret = real_pthread_cond_destroy(&p_ctx->state_cond);
        ABTI_ASSERT(ret == 0);
        ret = real_pthread_mutex_destroy(&p_ctx->state_lock);
...
{
    p_ctx->native_thread = real_pthread_self();
}
