...
static int pool_init(ABT_pool pool, ABT_pool_config config)
{
    ...
    ret = real_pthread_mutex_init(&p_data->mutex, NULL);
    if (ret != 0) {
        ABTU_free(p_data);
        return ABT_ERR_SYS;
    }
    ret = real_pthread_cond_init(&p_data->cond, NULL);
    if (ret != 0) {
                           ...
                           size_t num_units, ABT_pool_context context)
{
    ...
    if (num_units > 0) {
        ...
        if (num_units == 1) {
            ...
            real_pthread_cond_broadcast(&p_data->cond);
        }
        real_pthread_mutex_unlock(&p_data->mutex);
    }
}
...
#else
        /* We cannot use real_pthread_cond_timedwait().  Let's use nanosleep()
         * instead */
